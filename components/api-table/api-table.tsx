...
const parseMarkdownLinks = (text: string) => {
  ...
  text.replace(regex, (match, p1, p2, p3, p4, offset) => {
    ...
    if (p1 && p2 && p3) {
      ...
      parts.push(<code key={offset} className="bg-grey2 p-1 rounded before:!content-none after:!content-none">{p4.slice(1, -1)}</code>);
    }
...
const ApiTable: React.FC<Props> = ({ columns, data }) => {
  return (
    <div className="mt-4 grid grid-cols-1 border overflow-hidden border-[#262626] !text-grey2">
      {/* Table Header */}
      <div className="grid grid-cols-12 bg-white/5 font-semibold">
      ...
      <div className="overflow-y-auto">
        {data.map((row, rowIndex) => (
          <div
            className="grid grid-cols-12 odd:bg-transparent even:bg-white/5 last:border-b-0 border-b border-[#262626]"
            key={rowIndex}
          >
