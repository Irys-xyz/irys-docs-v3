function findSnippetsAroundKeywords(text: string, keywords: string): string {
    ...
    individualKeywords.forEach((keyword) => {
        ...
        while (startIndex !== -1 && occurences < 4) {
            // Calculate start and end positions for the snippet
            const snippetStart: number = Math.max(startIndex - CHARACTER_AROUND, 0)
            const snippetEnd: number = Math.min(
                startIndex + keyword.length + CHARACTER_AROUND,
                text.length
            )

            // Determine whether ellipses should be added before/after the snippet
            const snippetPrefix: string = snippetStart > 0 ? '...' : ''
            const snippetSuffix: string = snippetEnd < text.length ? '...' : ''
            ...
            // If the next occurrence is within the current snippet, skip it
            if (startIndex !== -1 && startIndex < snippetEnd) {
                startIndex = lowerText.indexOf(lowerKeyword, snippetEnd)
            }

            // Limit the number of snippets to 3
            occurences++
        }
    })

    // Merge overlapping or consecutive snippets
    const mergedSnippets: string[] = mergeSnippets(snippets)
...
function mergeSnippets(snippets: string[]): string[] {
    ...
    const merged: string[] = [snippets[0]]
    for (let i = 1; i < snippets.length; i++) {
        const lastMerged: string = merged[merged.length - 1]
        const current: string = snippets[i]
